...
// The agent runs inside a per-task git worktree (cwd) of the client's theme repo
// with real tools (Read/Edit/Grep) plus two live MCP servers: figma (pulls a
// linked design) and shopify-dev (validates its own work). It edits files in
// place; lib/github.js reads them back from that worktree afterwards.
//
// Ported from the GitHub Actions version: cwd + task name/description are now
// function args (the server runs many tasks across many repos) instead of a
// hardcoded REPO_ROOT + module-level env constants.

const { query } = require("@anthropic-ai/claude-agent-sdk");
const { AGENT_MODEL, MAX_AGENT_TURNS, FIGMA_TOKEN } = require("./config");
...
// The file changes themselves are read from the worktree afterwards.
//   ctx: { worktree, taskName, taskDescription }
async function runAgent(ctx) {
  const { worktree, taskName, taskDescription } = ctx;

  const figmaUrl = detectFigmaUrl(`${taskName}\n${taskDescription}`);
  if (figmaUrl) {
